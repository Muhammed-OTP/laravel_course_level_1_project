        ...
        <div class="col-6 mx-auto">
          <form action="{{ route('login') }}" class="form-contact contact_form" action="contact_process.php" method="post"  novalidate="novalidate">
            ...
            <div class="form-group">
              ...
              @error('email')
              <span class="text-danger">{{ $message = $errors->first('email') }}</span>
              @enderror
            ...
            <div class="form-group">
              ...
              @error('password')
              <span class="text-danger">{{ $message = $errors->first('password') }}</span>
              @enderror
              {{-- <x-input-error :messages="$errors->get('password')" class="mt-2" /> --}}
            </div>
            <div class="form-group text-center text-md-right mt-3">
              <a href="{{ route('register') }}" class="button button--active button-contactForm">Register Instead!</a>
